...
// ListenAndServe - same as http.ListenAndServe(addr string, handler http.Handler) error unless it ads writeTimeout parameter and ensure tcp keep alive to prevent the client from timing out. Typically writeTimeout takes high value (i.e time.Hour) to ensure the tarpit is effective.
